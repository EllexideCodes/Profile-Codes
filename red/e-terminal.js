...
go();setInterval(function(){context.globalAlpha=0.2;context.drawImage(bgcanv,0,0);context.globalAlpha=1;sn.go();},1000/30);var input=$("#inputcmd");var output=$("#output");input.keypress(keypressInput);$("#terminal").click(openKeyboard);input.click(()=>{input.html("");input.focus();})
function keypressInput(e){if(e.keyCode==13){var command=input.text();output.html(proccessCMD(command));input.html("");e.preventDefault();}}
